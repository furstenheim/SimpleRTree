...
// ...
func bucketsX32(slice xSorter32, bucketSize int, buffer []int) {
	left := 0
	right := slice.Len() - 1
	stack := buffer[:0]
	...
	for len(s) > 0 {
		...
		selectX32(slice, mid, left, right)
...
// when left = 0
func selectX32(array xSorter32, k, left, right int) {
	length := array.Len()
	for right > left {
		if right-left > 600 {
			...
			var newLeft = xSorterMax32(left, int(math.Floor(kf-m*s/n+sd)))
			var newRight = xSorterMin32(right, int(math.Floor(kf+(n-m)*s/n+sd)))
			selectX32(array, k, newLeft, newRight)
		}
...
type xSorterStack32 []int

func (s xSorterStack32) push(v int) xSorterStack32 {
	return append(s, v)
}
func (s xSorterStack32) pop() (xSorterStack32, int) {
	l := len(s)
	return s[:l-1], s[l-1]
}
