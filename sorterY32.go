...
// ...
func bucketsY32(slice ySorter32, bucketSize int, buffer []int) {
	left := 0
	right := slice.Len() - 1
	stack := buffer[:0]
	stack = append(stack, left)
	stack = append(stack, right)
	s := ySorterStack32(stack)
	var mid int
	for len(s) > 0 {
		s, right = s.pop()
		s, left = s.pop()
		...
		selectY32(slice, mid, left, right)
...
// when left = 0
func selectY32(array ySorter32, k, left, right int) {
	length := array.Len()
	for right > left {
		if right-left > 600 {
			...
			var newLeft = ySorterMax32(left, int(math.Floor(kf-m*s/n+sd)))
			var newRight = ySorterMin32(right, int(math.Floor(kf+(n-m)*s/n+sd)))
			selectY32(array, k, newLeft, newRight)
		}
...
type ySorterStack32 []int

func (s ySorterStack32) push(v int) ySorterStack32 {
	return append(s, v)
}
func (s ySorterStack32) pop() (ySorterStack32, int) {
	l := len(s)
	return s[:l-1], s[l-1]
}
